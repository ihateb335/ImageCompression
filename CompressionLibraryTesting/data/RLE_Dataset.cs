...
namespace CompressionLibraryTesting
{
    internal class RLE_Dataset : Dataset
    {
        ...
        public static RLE_Dataset Data_7 => new RLE_Dataset {
            RawData = new byte[129].Select((x, i) => (byte)((i + 1) % 128)).ToArray(),
            RawResultData = new byte[] { 127 }.Concat(new byte[128].Select((x, i) => (byte)((i + 1) % 128))).Concat(new byte[] { 0, 1}).ToArray(),
        };

        public static RLE_Dataset Data_8 => new RLE_Dataset {
            RawData = new byte[129].Concat(new byte[127].Select((x, i) => (byte)(i + 1))).ToArray(),
            RawResultData = new byte[] { 255, 0, 127, 0 }.Concat(new byte[127].Select((x, i) => (byte)(i + 1))).ToArray(),
        };
